...
Preprocess::Preprocess() : feature_enabled(0), blind(0.01), point_filter_num(1)
{
  inf_bound = 10;
  N_SCANS = 6;
  group_size = 8;
...
void Preprocess::process(const livox_driver::msg::CustomMsg::UniquePtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  livox_handler(msg);
  *pcl_out = pl_surf;
}

void Preprocess::livox_handler(const livox_driver::msg::CustomMsg::UniquePtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
          ...
          ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        pl_full[i].curvature =
            msg->points[i].offset_time / float(1000000);
